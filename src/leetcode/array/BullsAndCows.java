...
public class BullsAndCows {
    public String getHint(String secret, String guess) {
        ...
        for (int i = 0; i < secret.length(); i++) {
            ...
            if (charGuess == charSecret)    bulls++;
            else {
                int numberOfSecret = charSecret - '0';
                int numberOfGuess = charGuess - '0';
                if (count[numberOfSecret] < 0)  cows++;
                if (count[numberOfGuess] > 0)   cows++;
                count[numberOfSecret]++;
                count[numberOfGuess]--;
            }
        }
